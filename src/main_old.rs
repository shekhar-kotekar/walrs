...
async fn main() {
    ...
    let sleep_duration_seconds = Duration::from_secs(rand::thread_rng().gen_range(5..30));
    tracing::info!("sleep interval time is: {:?}.", sleep_duration_seconds);

    let cluster_info: Cluster = get_cluster_info(sleep_duration_seconds);
...
fn get_cluster_info(sleep_duration_seconds: Duration) -> Cluster {
    let pod_uid = std::env::var("POD_UID").unwrap();
    let pod_ip: String = std::env::var("POD_IP").unwrap();

    let mut nodes_in_cluster: Vec<Node> =
        wait_until_nodes_are_added_to_cluster(pod_ip.as_str(), sleep_duration_seconds);
...
fn wait_until_nodes_are_added_to_cluster(
    pod_ip: &str,
