...
pub async fn send_heartbeat(self_address: String, cluster_info: ClusterInfo) {
    match cluster_info.brokers.get(&self_address) {
        Some(broker_info) => {
...
mod tests {
    ...
    async fn test_send_heartbeat() {
        ...
        let mut self_info = BrokerInfo::new();
        ...
        cluster_info.add_broker(remote_address.clone(), BrokerInfo::new());
        ...
        match common::read_command_from_socket::<CommandToPeer>(&mut remote_stream).await {
            Some(command) => {
            ...
            None => {
                heartbeat_task_join_handle.await.unwrap();
                panic!("Failed to read command from remote stream");
            }
        }
