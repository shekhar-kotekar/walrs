...
impl Broker {
    pub fn new(
        ...
        std::fs::create_dir_all(&data_dir_path).unwrap_or_else(|e| {
            panic!("Failed to create base directory: {} because: {}", &data_dir_path, e);
        });
    ...
    pub async fn start(&mut self, mut command_rx: Receiver<CommandToBroker>, cancellation_token: CancellationToken) {
        loop {
            tokio::select! {
